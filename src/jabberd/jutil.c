...
//return the timestamp according to XEP-0203
char *jutil_timestamp(void)
{
    ...
    ret = snprintf(timestamp, 21, "%d-%02d-%02dT%02d:%02d:%02dZ", 1900+new_time->tm_year,
...
//return timestamp according to XEP-0091, this method will be deleted in the future
char *jutil_timestamp_0091(void)
{
    time_t t;
    struct tm *new_time;
...
void jutil_error(xmlnode x, terror E)
{
    ...
    condition = xmlnode_insert_tag(err, E.condition);
    xmlnode_put_attrib(condition, "xmlns", NS_STANZA); 
    if(E.msg != NULL)
    {
        text = xmlnode_insert_tag(err, "text");
        xmlnode_put_attrib(text, "xmlns", NS_STANZA);
        xmlnode_put_attrib(text, "xml:lang", "en");
        xmlnode_insert_cdata(text,E.msg,strlen(E.msg));
    }
    jutil_tofrom(x);
}

//add delay timestamp to a stanza
